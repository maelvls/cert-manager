...
func GenerateTemplate(now time.Time, crt *v1.Certificate) (*x509.Certificate, error) {
	commonName := crt.Spec.CommonName
...
func GenerateTemplateFromCertificateRequest(now time.Time, cr *v1.CertificateRequest) (*x509.Certificate, error) {
	...
	return GenerateTemplateFromCSRPEMWithUsages(now, cr.Spec.Request, certDuration, cr.Spec.IsCA, keyUsage, extKeyUsage)
}

func GenerateTemplateFromCSRPEM(now time.Time, csrPEM []byte, duration time.Duration, isCA bool) (*x509.Certificate, error) {
	var (
		ku  x509.KeyUsage
		eku []x509.ExtKeyUsage
	)
	return GenerateTemplateFromCSRPEMWithUsages(now, csrPEM, duration, isCA, ku, eku)
}

func GenerateTemplateFromCSRPEMWithUsages(now time.Time, csrPEM []byte, duration time.Duration, isCA bool, keyUsage x509.KeyUsage, extKeyUsage []x509.ExtKeyUsage) (*x509.Certificate, error) {
